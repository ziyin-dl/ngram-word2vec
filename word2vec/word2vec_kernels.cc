...
==============================================================================*/
/* this kernel file is modified to operate on ngram data rather than the
 * corpus file. each line of the ngram data is of the format "ngram \t count \n"
 * where the ngram itself is white space separated. */
...
bool ScanWordWithLineBreak(StringPiece* input, string* word) {
...
class SkipgramWord2vecOp : public OpKernel {
 public:
  explicit SkipgramWord2vecOp(OpKernelConstruction* ctx)
    ...
    for (int i = 0; i < kPrecalc; ++i) {
      NextExampleWithBoundary(&precalc_examples_[i].input, &precalc_examples_[i].label);
    }
  }
      ...
      for (int i = 0; i < batch_size_; ++i) {
        ...
        if (precalc_index_ >= kPrecalc) {
          ...
          for (int j = 0; j < kPrecalc; ++j) {
            NextExampleWithBoundary(&precalc_examples_[j].input,
 ...
 private:
  ...
  std::vector<std::vector<int> > corpus_sep_by_sentence_;
  std::vector<int> ngram_count_;
  ...
  int32 example_sentence_ GUARDED_BY(mu_) = 0;
  int32 example_sentence_count_ GUARDED_BY(mu_) = 1;
  int32 example_pos_ GUARDED_BY(mu_);
  int32 label_pos_ GUARDED_BY(mu_);
  int32 label_limit_ GUARDED_BY(mu_);

  // example_sentence_ is the cursor for the sentence of the next example.
  // {example_pos_, label_pos_} is the cursor for the next example in the next sentence.
  // example_pos_ wraps around at the end of corpus_. For each
  // example, we randomly generate [label_pos_, label_limit) for
  // labels.
  void NextExampleWithBoundary(int32* example, int32* label) EXCLUSIVE_LOCKS_REQUIRED(mu_) {
    while (true) {
      if (label_pos_ >= label_limit_) {
        ...
        if (sentence_index_ >= sentence_.size()) {
          ...
          sentence_index_ = 0;
          example_pos_ = 0;
          example_sentence_count_ --;
          if (example_sentence_count_ <= 0) {
            example_sentence_ ++;
            example_sentence_count_ = ngram_count_[example_sentence_];
          }
          if (example_sentence_ >= corpus_sep_by_sentence_.size()) {
            example_sentence_ = 0;
            current_epoch_ ++;
          }
          for (int i = 0; i < corpus_sep_by_sentence_[example_sentence_].size(); ++i, ++example_pos_) {
            if (example_pos_ >= corpus_sep_by_sentence_[example_sentence_].size()) {
              example_pos_ = 0;
              break;
            }
            if (subsample_ > 0) {
              int32 word_freq = freq_.flat<int32>()(corpus_sep_by_sentence_[example_sentence_][example_pos_]);
            ...
            sentence_.push_back(corpus_sep_by_sentence_[example_sentence_][example_pos_]);
          }
        }
        const int32 skip = 1 + rng_.Uniform(window_size_);
        label_pos_ = std::max<int32>(0, sentence_index_ - skip);
        label_limit_ =
            std::min<int32>(sentence_.size(), sentence_index_ + skip + 1);
      }
    ...
    //std::cout << "total: " << sentence_.size() << " with sample: "
    //  << sentence_index_ << ", " << label_pos_ << std::endl;
  ...
  Status Init(Env* env, const string& filename) {
    ...
    string w;
    corpus_size_ = 0;
    std::vector<std::vector<string> > corpus_raw;
    corpus_raw.emplace_back(std::vector<string>());
    while (ScanWordWithLineBreak(&input, &w)) {
      /* note the format of ngram count file: last word in line 
       * is the number of count. If greater than 1, we should 
       * repeat this sample to match the count */
      // std::cout << w << std::endl;
      if (w == "\n") {
        int count = std::stoi(corpus_raw.back().back());
        corpus_raw.back().pop_back();
        ngram_count_.push_back(std::max(1, count));
        corpus_raw.emplace_back(std::vector<string>());
      } else {
        corpus_raw.back().push_back(w);
      }
    }
    corpus_raw.pop_back();
    std::cout << "sizes: " << corpus_raw.size() << ", " << ngram_count_.size() << std::endl;
    assert(corpus_raw.size() == ngram_count_.size());
    ...
    assert(corpus_sep_by_sentence_.size() == ngram_count_.size());
    /* free up memory */
    corpus_raw.clear();
    std::vector<std::vector<string> >(corpus_raw).swap(corpus_raw);
