...
class Word2Vec(object):
  ...
  def save_vocab(self):
    ...
    with open(os.path.join(opts.save_path, "vocab_{}.txt".format(model_name)), "w") as f:
...
def main(_):
  ...
  with tf.Graph().as_default(), tf.Session() as session:
    ...
    # remove directories and suffix of the data file
    model_name = (opts.train_data.split('/')[-1]).split('.')[0]
    model.saver.save(session, os.path.join(opts.save_path, "model_{}.ckpt".format(model_name)),
